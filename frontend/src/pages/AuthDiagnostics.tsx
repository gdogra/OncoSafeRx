    ...
    try {
      ...
      if (session?.user?.id) {
        try {
          ...
          let q: any = supabase.from('users').select('id,email,role,first_name,last_name,specialty,institution,license_number,preferences,created_at').eq('id', session.user.id).maybeSingle();
          let result: any = await withTimeout(q, 4000);
          if (result?.error && /column/i.test(result.error.message)) {
            q = supabase.from('users').select('id,email,role,first_name,last_name').eq('id', session.user.id).maybeSingle();
