...
const AuthPage: React.FC = () => {
  ...
  // Google OAuth — redirects to /auth/callback on success
  const handleGoogleSignIn = async () => {
    try {
      await actions.signInWithGoogle();
      // signInWithGoogle triggers a redirect, so no further action needed here
    } catch (err) {
      // Error is surfaced via state.error by the auth context
    }
  };
        ...
        <Card className="py-8 px-4 shadow-lg sm:rounded-lg sm:px-10 border-0">
          ...
          {mode === 'signin' && (
            <form onSubmit={handleSignin} className="space-y-6">
              ...
              <button
                ...
                disabled={state.isLoading}
                className="w-full flex items-center justify-center gap-2 py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          {mode === 'signup' && (
            <form onSubmit={currentStep === 1 ? (e) => { e.preventDefault(); handleSignupNext(); } : handleSignupSubmit} className="space-y-6">
              ...
              {/* Google Sign Up (shown on step 1 only) */}
              {currentStep === 1 && (
                <>
                  <div className="relative my-4">
                    <div className="absolute inset-0 flex items-center">
