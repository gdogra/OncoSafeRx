...
export default function ConnectivityStatus({ apiBase, align = 'right', compact = false, pollMs = 120000 }: Props) {
  const base = useMemo(() => {
    ...
    // CRITICAL OVERRIDE: Force direct Render URL due to Netlify proxy 502 timeouts
    if (typeof window !== 'undefined' && window.location?.hostname !== 'localhost') {
      console.log('🚨 ConnectivityStatus: FORCING direct Render URL due to Netlify proxy issues');
